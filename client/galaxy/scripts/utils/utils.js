...
export function setWindowTitle(title) {
    if (title) {
        window.document.title = `ImmPort Galaxy ${window.Galaxy.config.brand ? ` | ${window.Galaxy.config.brand}` : ""} | ${_l(
            title
        )}`;
    } else {
        window.document.title = `ImmPort Galaxy ${window.Galaxy.config.brand ? ` | ${window.Galaxy.config.brand}` : ""}`;
    }
}
